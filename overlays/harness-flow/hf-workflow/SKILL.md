---
name: hf-workflow
description: HarnessFlow 主工作流入口。凡开发新功能、修改行为、修复缺陷、从想法搭建应用，或用户提到开始开发/继续/恢复进度/harness-flow 时，必须先加载本技能。主链为 grill-with-docs → to-product-architecture → to-spec → to-architecture → to-tickets → implement → ship，横切 hf-review。支持 interactive/auto。有 UI 时加载 hf-ui-design（含设计层：方向先行 + DESIGN.md + 截图评审）。不适用于纯问答、只读代码等无代码变更请求。
---
